from tkinter import *
# Initialize Tk and get a reference to the top-level window it automatically creates.
main_window = Tk()
# Create a label which displays the text "Count: 0", and place it into the top-level window.
count_label = Label(main_window, text="Count: 0") 
count_label.grid(row=0, column=1)
# The value of the counter starts at 0.
count_value = 0
# This function is called whenever the "Increment" button (defined below) is clicked
def increment_count():
...
# Whenever the button is clicked, it executes as a command the function 'incrementCount'.
